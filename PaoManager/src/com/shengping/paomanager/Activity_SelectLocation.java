...
import com.shengping.paomanager.adapter.Adapter_SelectLocation;
import com.shengping.paomanager.adapter.Adapter_SelectLocation.ViewHolder;
...
public class Activity_SelectLocation  extends Activity implements OnClickListener,OnItemClickListener,MyHttpCallBack{
	...
	private ImageView title_img_1,title_img_2;
	...
	private String type;//标识类型  起点  还是终点
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		lable_title=(RelativeLayout)findViewById(R.id.lable_title);
		lable_title.setOnClickListener(this);
		lable_input=(RelativeLayout)findViewById(R.id.lable_input);
		edit_address=(EditText)findViewById(R.id.edit_address);
		edit_address.addTextChangedListener(watcher);
		lable_select=(LinearLayout)findViewById(R.id.lable_select);
		mylist_select_pop=(ListView)findViewById(R.id.mylist_select);
		mylist_select_pop.setOnItemClickListener(this);
		title_img_1=(ImageView)findViewById(R.id.title_img_1);
		title_img_2=(ImageView)findViewById(R.id.title_img_2);
		this.title=getIntent().getStringExtra("title");
		if(type.equals("end")){
			title_img_1.setImageResource(R.drawable.ico_end_small);
			title_img_2.setImageResource(R.drawable.ico_end_small);
		}else if(type.equals("location")){
			title_img_1.setImageResource(R.drawable.ico_location_color);
		}
		title.setText(this.title);edit_address.setHint(this.title);
		...
		if(latitude!=0&&longitude!=0){
            ...
            popWin(current_latlng);
            isFirstLoc = false;
            MapStatusUpdate u = MapStatusUpdateFactory.newLatLng(current_latlng);
	    ...
	    */ 
	    ...
	    public boolean onKeyDown(int keyCode, KeyEvent event) { 
	        if ((keyCode == KeyEvent.KEYCODE_BACK)) { 
	            if(lable_select.getVisibility()==View.VISIBLE){
	            	lable_select.setVisibility(View.GONE);
	            	lable_input.setVisibility(View.GONE);
	            	lable_title.setVisibility(View.VISIBLE);
	            	return true;
	            }  
	             return false; 
	        }else { 
	...
	public void onClick(View v) {
		if(v.getId()==R.id.btn_back){
			finish();
		}else if(v.getId()==R.id.lable_title){
			lable_title.setVisibility(View.GONE);
			lable_input.setVisibility(View.VISIBLE);
			edit_address.setFocusable(true);
			edit_address.setFocusableInTouchMode(true);
			edit_address.requestFocus();
			lable_select.setVisibility(View.VISIBLE);
		}
	}
	@Override  
...
//            Log.i("BaiduLocationApiDem", sb.toString());
			...
			if(list!=null&&list.size()>0){
				if(!current_location.equals("")){
					for(PoiInfo info:list){
						if(info.address.equals(current_location)){
							popWin(info.location) ;
						}
					}
				}
				adapter=new Adapter_SelectLocation(Activity_SelectLocation.this, list,current_location);
	...
	public void popWin(LatLng point) {
		...
		if(type.equals("end")){
			bitmap= BitmapDescriptorFactory
					.fromResource(R.drawable.ico_end_large);
		}else if(type.equals("location")){
			bitmap= BitmapDescriptorFactory
					.fromResource(R.drawable.ico_location_color);
		}else{
		bitmap= BitmapDescriptorFactory
				.fromResource(R.drawable.ico_start_large);
		}
	...
	public void onItemClick(AdapterView<?> arg0, View view, int position, long arg3) {
		...
		String address=adapter.getItem(position).address;
		Bundle bundle=new Bundle();
		bundle.putString("address", address+" "+adapter.getItem(position).name);
